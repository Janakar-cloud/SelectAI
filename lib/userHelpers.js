...
async function findRegistrationConflict(emailLower, phoneNorm, excludeUid) {
  const userEmail = await User.findOne({ email: emailLower }).lean();
  if (userEmail && userEmail.uid !== excludeUid)
    return { message: 'An account with this email already exists.' };

  if (phoneNorm) {
    const userPhone = await User.findOne({ phone: phoneNorm }).lean();
    if (userPhone && userPhone.uid !== excludeUid)
      return { message: 'An account with this mobile number already exists.' };
  }
  ...
  if (phoneNorm) {
    const pendingPhone = await OtpVerification.findOne({
      phone: phoneNorm,
      verified: false,
      passwordHash: { $nin: ['', null] },
      ...(excludeUid ? { uid: { $ne: excludeUid } } : {})
    ...
    if (pendingPhone)
      return { message: 'Registration is already in progress for this mobile number.' };
  }
